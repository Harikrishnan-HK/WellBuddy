...
export default function Fitness() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([t, w, st, ct]) => { setToday(t); setWeek(w); setStepsTrend(st); setCalTrend(ct); })
      .catch(console.error)
  ...
  const weekWorkoutCount = allWorkouts.length;
  ...
  const avgSteps = stepsTrend.length > 0
    ? Math.round(stepsTrend.reduce((s, r) => s + r.value, 0) / stepsTrend.length)
    : null;
    ...
    <div className="px-4 pt-6 pb-4 space-y-5">
      ...
      <div className="bg-[#1e293b] rounded-2xl p-4">
        ...
        <div className="grid grid-cols-3 gap-3 text-center">
          <div>
            <p className="text-xl font-bold text-indigo-400">{weekWorkoutCount}</p>
            <p className="text-xs text-slate-400">Workouts</p>
          </div>
          <div>
            <p className="text-xl font-bold text-orange-400">{Math.round(weekCalories)}</p>
            <p className="text-xs text-slate-400">kcal burned</p>
          </div>
          <div>
            <p className="text-xl font-bold text-cyan-400">{Math.round(weekMinutes)}</p>
            <p className="text-xs text-slate-400">min active</p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-2">
          ...
          {[...allWorkouts].reverse().slice(0, 5).map((w, i) => (
            <div key={i} className="bg-[#1e293b] rounded-2xl p-4 flex items-center gap-3">
              <span className="text-2xl w-10 h-10 flex items-center justify-center bg-slate-800 rounded-xl">
              ...
              {w.avg_heart_rate && (
                <div className="text-right">
                  <p className="text-sm font-bold text-rose-400">{Math.round(w.avg_heart_rate)}</p>
                  <p className="text-[10px] text-slate-500">bpm avg</p>
                </div>
              )}
            </div>
          ))}
      ...
      {allWorkouts.length === 0 && stepsTrend.length === 0 && (
        <div className="bg-[#1e293b] rounded-2xl p-4 border border-dashed border-slate-600 text-center">
          <p className="text-sm text-slate-400">No fitness data yet.</p>
          <p className="text-xs text-slate-500 mt-1">Workouts and steps sync from Apple Health.</p>
        </div>
      )}
    </div>
  );
