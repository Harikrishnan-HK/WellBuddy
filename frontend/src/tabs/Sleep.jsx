...
export default function Sleep() {
  const fetch = useCallback(() => Promise.all([api.getToday(), api.getTrend('sleep_hours'), api.getTrend('hrv')]), []);
  const { data: raw, loading, refreshing, refresh } = useRefresh(fetch);
  const [today, sleepTrend, hrvTrend] = raw || [null, [], []];
  ...
  return (
    <div className="px-4 pt-6 pb-4 space-y-5">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <MetricCard icon="💓" label="HRV" value={hrv ? Math.round(hrv) : null} unit="ms"
          sub={avgHrv ? `30d avg ${avgHrv} ms` : undefined} color="text-rose-400" />
